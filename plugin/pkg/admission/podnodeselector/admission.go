...
// ymjx: .2 PodNodeSelector准入控制器
// PodNodeSelector准入控制器通过读取命名空间注释和准入控制器 配置文件来限制Pod资源对象可在命名空间内使用的节点选择器。
// PodNodeSelector准入控制器会对拦截的kube-apiserver请求中的Pod 资源对象进行修改，
// 将节点选择器与Pod资源对象的节点选择器进行合并并赋值给Pod资源对象的节点选择器（即pod.Spec.NodeSelectore）。
// 行合并并赋值给Pod资源对象的节点选择器（即pod.Spec.NodeSelectore）。
//
// # Pod资源对象的节点选择器（即pod.Spec.NodeSelector）必须为 true， 才能使Pod资源对象选择到合适的节点
//
// PodNodeSelector准入控制器在执行变更操作时，shouldIgnore函数会忽略Pod以外的资源对象，因为PodNodeSelector淮入控制器只对Pod资源对象有效。
// 通过p.WaitForReady函数判断该准入控制器是否已完成初始化，通过p.getNamespaceNodeSelectorMap函数选择节点选择器(namespaceNodeSelector），
// 它是通过读取命名空间注释和配置文件来选择节点选择器的。其执行过程如下。
// - 如果命名空间中具有带键的注释（即 scheduler.alpha.kubernetes.io/node-selector），则将其值用作节点选择器。
// - 如果命名空间中没有这样的注释（即scheduler.alpha.kubernetes.io/node-selector），则使用准入控制器配置文件中定义的clusterDefaultNodeSelector作为节点选择器。
// 通过labels.Conflicts函数判断节点选择器与资源对象的节点选 择器是否存在冲突。
// 如果存在冲突，则通过errors.NewForbidden函数 返回HTTP 403 Forbidden；
// 如果不存在冲突， 最后通过labels.Merge 函数将节点选择器与资源对象的节点选择器进行合并并赋值给Pod资源 对象的节点选择器（即pod.Spec.NodeSelectore）。
// 执行完PodNodeSelector准入控制器变更操作以后执行验证操作
